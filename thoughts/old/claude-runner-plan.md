...
**Output**: `SpecDocument` object stored at `.haflow/missions/<id>/spec.json`
...
async function humanReviewAndApprove(specDoc: SpecDocument): Promise<SpecDocument> {
  ...
  await saveToFile(specDoc, '.haflow/missions/<id>/spec-pending.json');
...
**Output**: `ResearchData` stored at `.haflow/missions/<id>/research.json`
...
async function humanReviewResearch(researchData: ResearchData): Promise<ResearchData> {
  await saveToFile(researchData, '.haflow/missions/<id>/research-pending.json');
...
**Output**: `ImplementationPlan` stored at `.haflow/missions/<id>/plan.json`
...
async function humanApproveAndSelectOneshot(
  ...
  await saveToFile(plan, '.haflow/missions/<id>/plan-pending.json');
...
All intermediate states saved to `.haflow/missions/<id>/`:
```
.haflow/missions/<mission-id>/
