...
2. When `workspaceMode: 'codegen'`:
   ...
   - Artifacts still available at `/workspace/artifacts` (or `/mission/artifacts`)
3. Before starting a codegen step, system checks for uncommitted changes and throws error if dirty
4. Existing document-processing workflows continue to work unchanged
5. Before stating a codegen step it will validate there is a LINKED project and linked project will also appear in the UI
...
- Automatic git commit after codegen 

## What We're NOT Doing 

- NOT Cloning the project via Giat (use linked project directly)
- NOT Mission-level project override (step uses global linkedProject)
- Multiple linked projects (single global linkedProject for now)
...
export interface ClaudeSandboxOptions {
  ...
  prompt: string;
  workspacePath?: string;  // NEW - if set, mount at /workspace and use as working dir
}
```
...
## Phase 2: Backend Config - Read linkedProject

### Overview
Enable the backend to read the `linkedProject` path from the CLI config file (`~/.haflow/config.json`). This bridges the gap between CLI and backend.

### Changes Required:

#### 1. Backend Config Utility
**File**: `packages/backend/src/utils/config.ts`
**Changes**: Add function to read linked project from CLI config
...
// Add to config object or as separate function
...
// Add git status check for codegen mode
export async function checkGitStatus(projectPath: string): Promise<{ clean: boolean; error?: string }> {
  try {
...
### Overview
Modify `startClaudeStreaming()` to conditionally mount the linked project at `/workspace` when `workspacePath` is provided. Also update working directory logic.
...
async function* startClaudeStreaming(options: ClaudeSandboxOptions): AsyncGenerator<StreamEvent, void, unknown> {
  ...
  // NEW: Mount workspace if in codegen mode
  if (workspacePath) {
...
#### Manual Verification:
- [ ] Start a container with workspacePath and verify `/workspace` is mounted
...
### Overview
Wire everything together: mission engine reads step's `workspaceMode`, fetches `linkedProject`, checks git status, and passes `workspacePath` to the sandbox.
...
```typescript
import { getLinkedProject, checkGitStatus } from '../utils/config.js';
...
You are working in the project directory at /workspace. The implementation plan is in /workspace/artifacts/implementation-plan.md.

Your task:
1. Read artifacts/implementation-plan.md to understand what to build
2. Implement each task in the plan by modifying files in the project
3. Write tests as specified
4. Ensure code quality and follows project conventions
5. Document what was done in "artifacts/implementation-result.json" with format:
   {
...
- [ ] Verify Claude works in `/workspace` directory
...
5. Test error cases:
   ...
   - Dirty git state -> error with clear message

## Performance Considerations

- Git status check adds ~500ms before codegen steps (acceptable)
