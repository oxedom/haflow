...
Implement the complete Vitest test suite for `@haflow/backend` covering 8 test files across utils, services, and integration layers. Tests will use **real Docker containers** and the integration tests will hit an **actual running Express server**.
...
1. All 8 test suites pass with `pnpm --filter @haflow/backend test`
2. Coverage report available via `pnpm --filter @haflow/backend test:coverage`
...
export async function startServer(port: number) {
  ...
  return app.listen(port, () => {
    console.log(`haflow backend listening on port ${port}`);
  });
}
```
...
- [x] `pnpm --filter @haflow/backend add -D supertest` completes successfully
...
- [x] Vitest config valid: `pnpm --filter @haflow/backend test` runs (0 tests found is OK)
...
#### Automated Verification:
- [x] All utils tests pass: `pnpm --filter @haflow/backend test tests/unit/utils`
...
#### Automated Verification:
- [x] Workflow tests pass: `pnpm --filter @haflow/backend test tests/unit/services/workflow`
...
#### Automated Verification:
- [x] Mission store tests pass: `pnpm --filter @haflow/backend test tests/unit/services/mission-store`
- [x] No TypeScript errors
...
#### Automated Verification:
- [x] Docker tests pass: `pnpm --filter @haflow/backend test tests/unit/services/docker`
- [x] No orphaned containers after test run: `docker ps -a --filter="label=haflow.mission_id"`
...
#### Automated Verification:
- [x] Mission engine tests pass: `pnpm --filter @haflow/backend test tests/unit/services/mission-engine`
- [x] No TypeScript errors
...
#### Automated Verification:
- [x] Integration tests pass: `pnpm --filter @haflow/backend test tests/integration`
- [x] All tests pass together: `pnpm --filter @haflow/backend test`
- [x] Coverage report generates: `pnpm --filter @haflow/backend test:coverage`
...
2. Run full test suite: `pnpm --filter @haflow/backend test`
3. Check no orphaned containers: `docker ps -a --filter="label=haflow.mission_id"`
4. Generate coverage report: `pnpm --filter @haflow/backend test:coverage`
