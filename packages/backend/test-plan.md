# Ralphy Backend Vitest Test Plan

## Overview

Test plan for the Ralphy v0 backend using Vitest. Tests are organized by layer following the implementation structure.
...
| `config.ralphyHome defaults to ~/.ralphy` | Uses homedir() |
| `config.ralphyHome respects RALPHY_HOME env` | Uses env value |
| `config.missionsDir is ralphyHome/missions` | Computed getter |
...
| Test Case | Description |
|-----------|-------------|
| `init creates missions directory if not exists` | Creates `~/.ralphy/missions/` |
...
| `cleanupOrphaned finds containers by label` | Filter by ralphy.mission_id |
...
pnpm --filter @ralphy/backend test mission-store
